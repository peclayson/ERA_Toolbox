function idtable = era_dep_ssrel(varargin)
%Calculate subject-level dependability using variance components from 
% CmdStan
%
...
bp = cell2mat(data.gro_sds(:,1)) .^ 2;
wp = (exp(data.pop_sdlog + cell2mat(data.ind_sdlog))) .^ 2;
...
idtable.pop_errvar = repmat(mean(exp(data.pop_sdlog).^2),...
    height(idtable), 1);
...
bp_std = mean(cell2mat(data.gro_sds(:,1)));
bp_var = bp_std^2;

pop_err_log = mean(data.pop_sdlog); 

pop_err_var = mean(exp(data.pop_sdlog) .^ 2); 
pop_err_std = sqrt(pop_err_var);


ss_err_log = cell2mat(data.ind_sdlog) + pop_err_log;

ss_err_var = mean(exp(ss_err_log) .^ 2,1);
ss_err_std = sqrt(ss_err_var);
